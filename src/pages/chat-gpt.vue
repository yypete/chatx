<template>
  <div class="flex">
          ...
          <a
            ...
            <div class="flex gap-3">
              <span class="flex items-center" data-state="closed"
                ><button
                  class="invisible hover:text-token-text-secondary group-hover:visible juice:text-token-text-secondary juice:hover:text-token-text-primary"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
    ...
    <NewChat :conversationId="conversationId" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const conversationId = ref<string>("123");
    function getId() {
      // 生成一个随机的字符串
      const randomString =
        Math.random().toString(36).substring(2, 15) +
        Math.random().toString(36).substring(2, 15);
      conversationId.value = randomString;
      chatHistory.value = getChatHistory(conversationId.value);
    }
    onMounted(() => {
      chatHistory.value = getChatHistory(conversationId.value);
    });
