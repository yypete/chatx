<template>
  <div class="w-full h-screen flex justify-center items-center">
    <div
      class="flex flex-col justify-center items-center chat-container relative h-full"
    >
      <!-- chat-with-robot -->
      ...
      >
        ...
        <input
          type="text"
          class="flex-1 mx-2 px-4 py-2 text-gray-700 placeholder-gray-400 bg-transparent border-none focus:outline-none"
          placeholder="Type a message to ironMan"
          v-model="userMessage"
          @keyup.enter="sendMessageHandler"
        />
        <button
          class="btn-right flex items-center justify-center h-full p-2 rounded-full text-gray-500"
...
import { sendMessage, getChatHistory, storeChatHistory } from "../api/cozeApi";
import { talkData } from "../pages/dataConfig";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 初始化加载聊天记录
    onMounted(() => {
      chatHistory.value = getChatHistory(conversationId);
    });

    const sendMessageHandler = async () => {
      if (userMessage.value.trim() === "") return;
      const userChat: ChatHistoryItem = {
        role: "user",
      ...
      try {
        // 添加用户消息到历史记录
        chatHistory.value.push(userChat);
        // 发送请求
        ...
        // 存储更新后的聊天记录
        storeChatHistory(conversationId, chatHistory.value);
        // 清空输入框
        userMessage.value = "";
      } catch (error: any) {
